...
function order_by($columnName = '', $default = null)
{
    ...
    // }
    if ($sortColumnName == $columnName) {
        $parameters = array_merge(Input::except($except, '/admin/users'), array($orderType => $columnName));
        $icon       = Input::get('sort_up') ? 'chevron-up' : 'chevron-down' ;
    } elseif ($sortColumnName === false && $default == 'asc') {
        $parameters = array_merge(Input::except('/admin/users'), array('sort_down' => $columnName));
        $icon       = 'chevron-up';
    } elseif ($sortColumnName === false && $default == 'desc') {
        $parameters = array_merge(Input::except('/admin/users'), array('sort_up' => $columnName));
        $icon       = 'chevron-down';
    } else {
        $parameters = array_merge(Input::except($except, '/admin/users'), array('sort_up' => $columnName));
        $icon       = 'random';
    }
